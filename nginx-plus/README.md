...
## Configuring & Collecting details from Okta
  ...
  * Open in your browser: `https://nginxjdesai.okta.com/.well-known/openid-configuration`
    ...
    * Obtain the URL for the **jwks_uri**
      - In my case "jwks_uri":"https://nginxjdesai.okta.com/oauth2/v1/keys"
...
Final checks to ensure we are all set. 
  * **write_permission to `/etc/nginx/conf.d` directory** - ensure that the tokens can be written to this directory by granting necessary permissions.
  * NGINX Reload - **sudo nginx -t & sudo nginx -s reload**

## Troubleshooting

Any errors generated by the OpenID Connect flow are logged to the error log, `/var/log/nginx/error.log`. Check the contents of this file as it may include error responses received by Okta. The level of detail recorded can be modified by adjusting the severity level of the `error_log` directive.

For more specific details on what to look for, please review the parent blog [nginx-openid-connect](https://github.com/nginxinc/nginx-openid-connect)

### Credits

The man, the machine, the guru - [Rajesh B.] (https://github.com/b-rajesh)
